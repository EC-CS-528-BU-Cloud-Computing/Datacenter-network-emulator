        ...
        for (dest_container, dest_ip) in zip(host_names, host_ips):
            if dest_ip != src_ip:
                ...
                try:
                    ...
                    sp.check_output(['docker', 'exec', "-d", dest_container, "nc", "-u", '-l','-p', "9998"]).decode("utf-8").strip()
                    # Source start UDP connection
                    # If the connection cannot be built it will throw Error
                    sp.check_output(['docker', 'exec', "-it", client_container,  "nc", "-z","-u", "-v", dest_ip, "9998"])
