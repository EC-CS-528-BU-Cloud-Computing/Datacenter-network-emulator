        ...
        for (dest_container, dest_ip) in zip(host_names, host_ips):
            if dest_ip != src_ip:
                ...
                try:
                    ...
                    sp.check_output(['docker', 'exec', "-d", dest_container, "nc", '-l', "-p", "9999"]).decode("utf-8").strip()
                    # Source start tcp connection
                    # If the connection cannot be built it will throw Error
                    sp.check_output(['docker', 'exec', "-it", client_container,  "nc", "-z", "-v", dest_ip, "9999"])
